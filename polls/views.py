...
def rating(request):
	...
	latest_question_list = Question.objects.annotate(num_answer=Count("answer")).order_by('-question_rating')[:100]
	pages = pagination(request, latest_question_list)
	context.update(pages)
	context['profile'] = get_user_data(request)
	context['authors'] = Profile.objects.order_by('-user_rating')[:10]
...
def tags(request, tag_id):
	...
	latest_question_list = question_list.order_by('-question_rating')
	pages = pagination(request, latest_question_list)
	context.update(pages)
...
def like(req):
	...
	import json
	content =json.dumps({'status': status, 'q_change': p_change , 'message': "10", 'new_rating': new_rate})
